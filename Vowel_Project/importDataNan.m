...
for i = 1:classes
    ...
    res(res==0) = NaN;
    columnMeans(i,:) =  mean(res,1,'omitnan');
    tempCovMatrix = cov(res,'omitrows');
...
for k =  1:length(testSet)
    xk = testSet(k,:);
    nanCols = (xk == 0); % find columns in xk with all 0s
    xk(:,nanCols) = []; 
    tempColumnMeans = columnMeans;
    tempColumnMeans(:,nanCols) = [];
    features_without_nan = length(xk)
    tempCovMatrices = covMatrcies;
    tempCovMatrices(:,nanCols) = [];
    tempCovMatrices(nanCols,:) = []; 
    pdf_k = zeros(1,classes);
    for C = 1:classes 
        tempCovMatrix = covMatrcies(1+features*(C-1):features*C,:);
        tempCovMatrix(:,nanCols) = [];
        tempCovMatrix(nanCols,:) = [];
        pdf_k(C) = mvnpdf(xk,tempColumnMeans(C,:),tempCovMatrix);
    end
